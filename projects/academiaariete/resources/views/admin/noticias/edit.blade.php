@extends('layouts.app')
@section('title','Admin - Editar noticia')
...
<section class="matricula-page">
  ...
  <form action="{{ route('admin.noticias.update', $noticia) }}" method="POST"
    ...
    <p class="mb-6 text-sm text-slate-600">
      Modifica la información de la noticia. Puedes dejarla en borrador o mantenerla publicada.
    </p>
    ...
    <div class="grid gap-4 md:grid-cols-1">
      ...
      <div>
        ...
        <textarea id="resumen" name="resumen" rows="3"
                  ...
                  aria-describedby="@error('resumen') resumen_error @enderror">{{ old('resumen', $noticia->resumen) }}</textarea>
      ...
      <div>
        ...
        <textarea id="contenido" name="contenido" rows="10" required
                  ...
                  aria-describedby="@error('contenido') contenido_error @enderror">{{ old('contenido', $noticia->contenido) }}</textarea>
    ...
    <div>
      ...
      {{-- Imagen actual fuera del recuadro --}}
      @if ($noticia->imagen)
        <div class="mb-3">
          ...
          <img src="{{ asset('storage/'.$noticia->imagen) }}"
               ...
               class="w-full max-w-xl h-auto rounded-lg border border-slate-200">
        </div>
      @endif

      <div class="file-card mt-2">
        <div class="file-title">
          <span><strong>Cambiar imagen (JPG, JPEG, PNG, WEBP)</strong></span>
        </div>
        ...
        <p class="hint">
          Si no seleccionas ninguna imagen, se mantendrá la actual. Tamaño máx. 2&nbsp;MB.
        </p>
    ...
    {{-- =======================
       ...
       ======================= --}}
    @php
      $estadoActual = $noticia->publicado_en ? 'publicar' : 'borrador';
